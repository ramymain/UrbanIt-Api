...
export class ProfileRepository extends Repository<Profile> {
    ...
    public findOneById(idProfile: number): Promise<Profile> {
        return this.manager.findOne(Profile, {where: {id: idProfile}, relations: ['user', "teams", "sport", "teams.teamLeader", "teams.teamLeader.profile", "teams.profiles", "teams.match", "teams.match.teams", "teams.match.teams.profiles", "teams.match.teams.profiles.user", "teams.match.teams.teamLeader", "teams.match.teams.teamLeader.profile", "teams.match.scores", "teams.match.scores.profiles", "teams.match.scoreVerif"]});
    }
    ...
    public findBest(sport: Sport, take: number, skip: number): Promise<Profile[]> {
        return this.manager.find(Profile, {where: {sport: sport}, order: {ranking: "DESC"}, take: take, skip: skip, relations: ['user']});
    }
